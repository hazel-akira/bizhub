 ...
 * Mirrors Flutter customer app logic (akira_bites_client).
 */
...
const OrderApp = {
            ...
            .map((p) => {
                ...
                <div class="aspect-[72/100] bg-white rounded-[16px] overflow-hidden shadow-[0_4px_6px_-1px_rgba(0,0,0,0.12),0_2px_4px_-2px_rgba(0,0,0,0.12)] flex flex-col">
                    ...
                    <div class="flex-[3] p-[12px] flex flex-col justify-between">
                        <div class="text-[14px] font-semibold text-[#1A1A1A] leading-[1.2] akira-two-line-clamp">
                            ${p.name}
                        </div>
        ...
        if (items) {
          ...
          <div class="bg-white rounded-2xl shadow-sm p-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <button type="button" data-product-id="${i.product_id}" data-action="dec" class="w-9 h-9 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center font-bold">−</button>
              <span class="w-8 text-center font-semibold">${i.quantity}</span>
              <button type="button" data-product-id="${i.product_id}" data-action="inc" class="w-9 h-9 rounded-full bg-orange-500 text-white hover:bg-orange-600 flex items-center justify-center font-bold">+</button>
            </div>
          </div>
        `
                )
    ...
    launchWhatsAppForCart() {
        ...
        if (win) {
            this.clearCart();
            this.renderCart();
            // Move the user back to the menu like the mobile app "pop" action.
...
// Vite builds this file as an ES module; expose on `window` so inline Blade scripts can access it.
